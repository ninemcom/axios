...
module.exports = function ue4(config) {
  return new Promise(function dispatchXhrRequest(resolve, reject) {
    ...
    request.OnComplete.Add( success => {
      ...
      var responseData = !config.responseType || config.responseType === 'text' ? request.GetContentAsString() : JSON.parse(request.GetContentAsString());
      var response = {
        data: responseData,
    ...
    // Send the request
    request.SetContentAsString(JSON.stringify(requestData));
